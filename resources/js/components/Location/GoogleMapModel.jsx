...
const GoogleMapModel = ({ open, onClose, onConfirm }) => {
    ...
    const { latitude, longitude, address } = useContext(LocationContext);
    ...
    const { isLoaded: googleMapsLoaded, loadError } =
        useGoogleMaps(googleApiKey);

    useEffect(() => {
        if (open) {
            // Set initial state from context when dialog opens
            ...
            // If we have coordinates but no address, geocode it
    ...
    const handleMapLoad = useCallback(
        (map) => {
            mapRef.current = map;
            setIsMapLoaded(true);
            // Only geocode if we don't already have an address
            if (!locationName && selectedLocation.lat && selectedLocation.lng) {
                geocodeLatLng(selectedLocation.lat, selectedLocation.lng);
            }
        },
        [geocodeLatLng, locationName, selectedLocation]
    );
    ...
    const handleAutocompleteChange = useCallback((event, newValue) => {
        if (!newValue?.place_id || !window.google || !window.google.maps)
            return;
    ...
    const handleConfirm = useCallback(() => {
        ...
        onClose();
    }, [selectedLocation, locationName, onConfirm, onClose]);
