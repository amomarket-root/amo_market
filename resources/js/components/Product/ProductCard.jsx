...
const ProductCard = ({
    ...
    return (
        <StyledCard onClick={() => onCardClick(product.id)}>
            ...
            <CardContent style={{ padding: '10px', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '8px' }}>
                    ...
                    {product.count > 0 ? (
                        <div
                            style={{ display: 'flex', alignItems: 'center', background: theme.palette.primary.main, borderRadius: '4px', padding: '2px 4px', color: 'white' }}
                            onClick={(e) => e.stopPropagation()}
                        >
                            <IconButton size="small" onClick={(e) => { e.stopPropagation(); onDecrease(product.id); }} style={{ color: 'white', padding: '4px' }}>
                                <Remove />
                            </IconButton>
                            <Typography variant="body2" component="div" style={{ margin: '0 8px', color: 'white' }}>
                                {product.count}
                            </Typography>
                            <IconButton size="small" onClick={(e) => { e.stopPropagation(); onIncrease(product.id); }} style={{ color: 'white', padding: '4px' }}>
                                <Add />
                            </IconButton>
                        </div>
                    ) : (
